/**
 * GLM-4.6V V4 cross-model replication gate — frozen analysis.
 *
 * Implements docs/experiments/GLM46V_CROSS_MODEL_REPLICATION_ANALYSIS_FREEZE_V2.md
 * exactly. Written BEFORE any formal V4 outcome is inspected.
 *
 * Differences from the V3 analyzer (which remains as the V3 stop record):
 *  - a planned task marked failed/skipped in the manifest task status may lack
 *    a run file and is accounted as missing (failed_task / skipped_task),
 *    entering the all-44 worst/best bound; it no longer rejects the whole
 *    analysis;
 *  - completed tasks without a run file, tasks without a taskStatus entry,
 *    extra tasks, duplicate tasks, and task 14 remain fail-closed;
 *  - missing reasons are classified: failed_task | skipped_task | null_brier |
 *    unexplained_missing;
 *  - the bound records assignedLowerDelta: -2, assignedUpperDelta: +2, and
 *    incompleteTaskCount: n.
 *
 * Primary estimand, bootstrap (10,000, seed 0x5EED0F), PASS/DIRECTIONAL/FAIL
 * classification, secondary analyses, and the post-run order are identical to
 * the frozen V1 spec. Outputs analysis-v1.json + analysis-v1.md into the V4
 * output directory, never silently overwriting (--verify recomputes).
 */
...
import {
  ...
  type Glm46vTwoArmManifestV4,
  type Glm46vTwoArmSummaryV4,
...
export function createV4SeededRng(seed: number): () => number {
  let a = seed >>> 0;
  return () => {
    a = (a + 0x6d2b79f5) >>> 0;
    let t = a;
...
export const V4_BOOTSTRAP_MASTER_SEED = 0x5eed0f;
export const V4_BOOTSTRAP_REPETITIONS = 10_000;
...
function bootstrapCis(values: number[], repetitions: number, seed: number): { lo: number; hi: number } | null {
  ...
  if (n === 0) return null;
  const rng = createV4SeededRng(seed);
  const means: number[] = [];
...
// ---------------------------------------------------------------------------
// Frozen population / artifact validation (V4 missingness rules)
// ---------------------------------------------------------------------------

export interface V4RunFileRows {
  taskId: number;
  rows: ForkRow[];
}

export interface V4AnalysisInput {
  plan: Glm46vTwoArmPlanV4;
  manifest: Glm46vTwoArmManifestV4;
  summary: Glm46vTwoArmSummaryV4;
  ledgerScan: Glm46vLedgerScanV1;
  /** Run files of COMPLETED tasks only. */
  runFiles: V4RunFileRows[];
}

export type V4MissingReason = "failed_task" | "skipped_task" | "null_brier" | "unexplained_missing";

/**
 * Strict population gate. A planned task marked failed/skipped may lack a run
 * file (accounted as missing); anything else inconsistent fails closed.
 */
export function validateV4Population(input: V4AnalysisInput): string[] {
  const problems: string[] = [];
  if (input.plan.seeds.length !== 1 || !Number.isSafeInteger(input.plan.seeds[0])) {
    problems.push(`plan must contain exactly one integer seed, got ${JSON.stringify(input.plan.seeds)}`);
  }
  const expectedSeed = input.plan.seeds[0];
  const planned = new Set(input.plan.taskIds);
  const statusByTask = new Map(input.manifest.taskStatus.map(entry => [entry.taskId, entry]));
  const filesByTask = new Map(input.runFiles.map(file => [file.taskId, file]));

  // every planned task must have exactly one taskStatus entry
  for (const taskId of input.plan.taskIds) {
    const status = statusByTask.get(taskId);
    if (!status) {
      problems.push(`task ${taskId}: no taskStatus entry (unexplained missing)`);
      continue;
    }
    if (status.status === "completed" && !filesByTask.has(taskId)) {
      problems.push(`task ${taskId}: taskStatus=completed but no run file`);
    }
    if (status.status !== "completed" && filesByTask.has(taskId)) {
      problems.push(`task ${taskId}: taskStatus=${status.status} but a run file exists`);
    }
  }
  // taskStatus entries outside the plan are inconsistent
  for (const entry of input.manifest.taskStatus) {
    if (!planned.has(entry.taskId)) {
      problems.push(`taskStatus entry for out-of-plan task ${entry.taskId}`);
    }
  }
  // run files: task 14 rejected; out-of-plan rejected; duplicates rejected
  ...
  for (const file of input.runFiles) {
    ...
    if (seen.has(file.taskId)) {
      problems.push(`task ${file.taskId}: duplicate run file`);
    }
    ...
    for (const row of file.rows) {
      if (row.seed !== expectedSeed) {
        problems.push(`task ${file.taskId}: seed ${row.seed} does not match frozen plan seed ${expectedSeed}`);
      }
...
export interface V4AnalysisV1 {
  ...
  primary: {
    ...
    missingPairs: number;
    missingReasons: Record<V4MissingReason, number>;
    meanDelta: number | null;
    ...
    bounds44: {
      assignedLowerDelta: number;
      assignedUpperDelta: number;
      incompleteTaskCount: number;
      lowerMeanDelta: number | null;
      upperMeanDelta: number | null;
    };
  };
  secondary: {
    ...
    severityBins: V4SeverityBin[];
  };
  taskTable: V4TaskDelta[];
  contentHash: string;
}

export function computeV4Analysis(input: V4AnalysisInput): V4AnalysisV1 {
  // ---- population gate ----
  const problems = validateV4Population(input);
  if (problems.length > 0) {
    throw new Error(`v4_analysis_population_rejected: ${problems.join("; ")}`);
  }

  // ---- per-task status + pair construction ----
  const statusByTask = new Map(input.manifest.taskStatus.map(entry => [entry.taskId, entry]));
  const filesByTask = new Map(input.runFiles.map(file => [file.taskId, file.rows]));
  const missingReasons: Record<V4MissingReason, number> = {
    failed_task: 0, skipped_task: 0, null_brier: 0, unexplained_missing: 0,
  };
  const deltas: V4TaskDelta[] = [];
  for (const taskId of input.plan.taskIds) {
    const status = statusByTask.get(taskId)!;
    const rows = filesByTask.get(taskId);
    const control = rows?.find(r => r.arm === "CONTROL") ?? null;
    const attacks = rows?.find(r => r.arm === "ATTACKS") ?? null;
    let complete = true;
    let missingReason: V4MissingReason | undefined;
    if (status.status === "failed") {
      complete = false;
      missingReason = "failed_task";
    } else if (status.status === "skipped") {
      complete = false;
      missingReason = "skipped_task";
    } else if (!control || !attacks) {
      complete = false;
      missingReason = "unexplained_missing";
    } else if (control.finalBrier === null || attacks.finalBrier === null) {
      complete = false;
      missingReason = "null_brier";
    }
    if (!complete) {
      missingReasons[missingReason!] += 1;
    }
    deltas.push({
      taskId,
  ...
  const ci = bootstrapCis(completeDeltas, V4_BOOTSTRAP_REPETITIONS, V4_BOOTSTRAP_MASTER_SEED);
  const meanDelta = completeDeltas.length > 0 ? mean(completeDeltas) : null;
  let classification: V4AnalysisV1["primary"]["classification"] = "UNCLASSIFIED";
  ...
  // ---- all-44-task conservative bounds (assignedLowerDelta -2 / +2) ----
  const incomplete = deltas.filter(d => !d.complete);
  const lowerVals = deltas.map(d => d.complete ? d.delta! : -2);
  const upperVals = deltas.map(d => d.complete ? d.delta! : +2);
  const bounds44 = {
    assignedLowerDelta: -2,
    assignedUpperDelta: +2,
  ...
  if (spInput.length >= 2) {
    const rng = createV4SeededRng(V4_BOOTSTRAP_MASTER_SEED);
    const rhos: number[] = [];
    for (let r = 0; r < V4_BOOTSTRAP_REPETITIONS; r++) {
      const xs: number[] = [];
      const ys: number[] = [];
  ...
  const body: Omit<V4AnalysisV1, "contentHash"> = {
    analysisSchemaRef: { id: "swarmalpha.experiment.v6-fork-glm46v-v4-analysis", version: "1.0.0" },
    inputs: {
    ...
    primary: {
      ...
      missingReasons,
      meanDelta,
      bootstrapRepetitions: V4_BOOTSTRAP_REPETITIONS,
      masterSeed: V4_BOOTSTRAP_MASTER_SEED,
    ...
    secondary: {
      accuracy: {
        ...
        ci95: bootstrapCis(accDeltas, V4_BOOTSTRAP_REPETITIONS, V4_BOOTSTRAP_MASTER_SEED),
      },
...
// ---------------------------------------------------------------------------
// Output read (only for the real V4 directory)
// ---------------------------------------------------------------------------

export function readV4OutputDirectory(outputDir: string = GLM46V_OUTPUT_DIR): V4AnalysisInput {
  const plan = JSON.parse(fs.readFileSync(path.join(outputDir, "plan.json"), "utf8")) as Glm46vTwoArmPlanV4;
  if (recomputeGlm46vPlanHash(plan) !== plan.contentHash) {
    throw new Error("v4_analysis_plan_hash_mismatch");
  }
  const rebuilt = buildGlm46vTwoArmPlanV4();
  if (plan.contentHash !== rebuilt.contentHash) {
    throw new Error("v4_analysis_plan_not_current: frozen plan.json no longer matches current runner constants");
  }
  const manifest = JSON.parse(fs.readFileSync(path.join(outputDir, "manifest.json"), "utf8")) as Glm46vTwoArmManifestV4;
  if (recomputeGlm46vManifestHash(manifest) !== manifest.contentHash) {
    throw new Error("v4_analysis_manifest_hash_mismatch");
  }
  if (manifest.planHash !== plan.contentHash) {
    throw new Error("v4_analysis_manifest_plan_mismatch");
  }
  const summary = JSON.parse(fs.readFileSync(path.join(outputDir, "summary.json"), "utf8")) as Glm46vTwoArmSummaryV4;
  if (recomputeGlm46vSummaryHash(summary) !== summary.contentHash) {
    throw new Error("v4_analysis_summary_hash_mismatch");
  }
  const ledgerScan = readGlm46vLedgerScan(outputDir);
  const runFiles: V4RunFileRows[] = [];
...
export function renderV4AnalysisMarkdown(analysis: V4AnalysisV1): string {
  const p = analysis.primary;
  const lines: string[] = [];
  lines.push("# GLM-4.6V V4 Cross-Model Replication Gate — Analysis V1");
  lines.push("");
  ...
  lines.push(`- all-44 worst/best bound (incomplete delta := ${p.bounds44.assignedLowerDelta}/${p.bounds44.assignedUpperDelta}, n=${p.bounds44.incompleteTaskCount}): [${p.bounds44.lowerMeanDelta === null ? "null" : p.bounds44.lowerMeanDelta.toFixed(6)}, ${p.bounds44.upperMeanDelta === null ? "null" : p.bounds44.upperMeanDelta.toFixed(6)}]`);
  lines.push("");
  ...
  lines.push("");
  lines.push("| taskId | status | CONTROL brier | ATTACKS brier | delta | CONTROL acc | ATTACKS acc | round-1 pooled brier |");
  lines.push("|---|---|---|---|---|---|---|---|");
  for (const t of analysis.taskTable) {
    lines.push(`| ${t.taskId} | ${t.complete ? "pair" : `missing(${t.missingReason})`} | ${t.controlBrier === null ? "-" : t.controlBrier.toFixed(6)} | ${t.attacksBrier === null ? "-" : t.attacksBrier.toFixed(6)} | ${t.delta === null ? "-" : t.delta.toFixed(6)} | ${t.controlAccuracy === null ? "-" : t.controlAccuracy} | ${t.attacksAccuracy === null ? "-" : t.attacksAccuracy} | ${t.round1PooledBrier === null ? "-" : t.round1PooledBrier.toFixed(6)} |`);
  }
  lines.push("");
  return lines.join("\n");
}

export function writeV4AnalysisOutput(
  outputDir: string,
  analysis: V4AnalysisV1,
): void {
  ...
  const mdText = renderV4AnalysisMarkdown(analysis);
  if (fs.existsSync(jsonPath)) {
    const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8")) as V4AnalysisV1;
    if (existing.contentHash !== analysis.contentHash) {
      throw new Error("v4_analysis_no_replace_conflict: analysis-v1.json exists with different content; refusing to overwrite");
    }
  } else {
  ...
  if (fs.existsSync(mdPath)) {
    ...
    if (!existing.includes(analysis.contentHash)) {
      throw new Error("v4_analysis_no_replace_conflict: analysis-v1.md exists with different content; refusing to overwrite");
    }
  } else {
...
export function verifyV4AnalysisOutput(outputDir: string = GLM46V_OUTPUT_DIR): { ok: boolean; detail: string } {
  const jsonPath = path.join(outputDir, "analysis-v1.json");
  const mdPath = path.join(outputDir, "analysis-v1.md");
  if (!fs.existsSync(jsonPath) || !fs.existsSync(mdPath)) {
    return { ok: false, detail: "analysis-v1.json or analysis-v1.md missing" };
  }
  const recomputed = computeV4Analysis(readV4OutputDirectory(outputDir));
  const existing = JSON.parse(fs.readFileSync(jsonPath, "utf8")) as V4AnalysisV1;
...
export async function runV4AnalysisCli(): Promise<number> {
  if (process.argv.includes("--verify")) {
    const result = verifyV4AnalysisOutput();
    console.log(JSON.stringify({ mode: "v4-analysis-verify", ok: result.ok, detail: result.detail }, null, 2));
    return result.ok ? 0 : 1;
  }
  const analysis = computeV4Analysis(readV4OutputDirectory());
  writeV4AnalysisOutput(GLM46V_OUTPUT_DIR, analysis);
  console.log(JSON.stringify({ mode: "v4-analysis", analysisContentHash: analysis.contentHash, classification: analysis.primary.classification }, null, 2));
  return 0;
}

if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  runV4AnalysisCli().then(code => { process.exitCode = code; }).catch(error => {
