 ...
 *   1. default V4/seed-1 variants use CONTROL + ATTACKS only; the explicit
 *      `three-arm-seed2` variant uses CONTROL + SUPPORTS + ATTACKS;
 ...
 *   4. Plan V4 frozen as plan.json (fail-closed, non-overwritable). The
 ...
 *
 * Output lands in a variant-specific separate directory (historical plan.json
 * files are never touched):
 *   experiments/campaign/pilot_output/v6-fork-glm46v-twoarm-v4-20260820
 *
 * Usage:
...
import {
  ...
  FORK_BELIEF_PARSER_REF,
  FORK_DISCUSSION_PROMPT_REF,
  forkPlannedCallsForRunV1,
  forkRunId,
  forkRunPath,
  runFork,
  V6ForkTaskIneligibleError,
  type ForkArm,
  type ForkRow,
} from "./run_v6_fork";
...
// ---------------------------------------------------------------------------
// Frozen gate constants (Plan V4)
// ---------------------------------------------------------------------------

export const GLM46V_EXPERIMENT_VARIANT = process.env.GLM46V_EXPERIMENT_VARIANT === "seed1-replication"
  ? "seed1-replication"
  : process.env.GLM46V_EXPERIMENT_VARIANT === "three-arm-seed2"
    ? "three-arm-seed2"
    : "seed0-v4";
const IS_SEED1_REPLICATION = GLM46V_EXPERIMENT_VARIANT === "seed1-replication";
const IS_THREE_ARM_EXTENSION = GLM46V_EXPERIMENT_VARIANT === "three-arm-seed2";

export const GLM46V_MODEL = "zhipu:glm-4.6v";
export const GLM46V_MODEL_ID = "glm-4.6v";
export const GLM46V_SEEDS: readonly number[] = IS_THREE_ARM_EXTENSION
  ? [2]
  : IS_SEED1_REPLICATION ? [1] : [0];
export const GLM46V_ARMS: readonly ForkArm[] = IS_THREE_ARM_EXTENSION
  ? ["CONTROL", "SUPPORTS", "ATTACKS"]
  : ["CONTROL", "ATTACKS"];
/**
 ...
 */
export const GLM46V_ARTIFACT_PATH = IS_THREE_ARM_EXTENSION
  ? "experiments/campaign/pilot_output/v6-fork-glm46v-threearm-seed2-20260821"
  : IS_SEED1_REPLICATION
    ? "experiments/campaign/pilot_output/v6-fork-glm46v-twoarm-seed1-v3-20260821"
    : "experiments/campaign/pilot_output/v6-fork-glm46v-twoarm-v4-20260820";
export const GLM46V_ANALYSIS_SPEC_PATH = IS_THREE_ARM_EXTENSION
  ? "docs/experiments/GLM46V_THREE_ARM_ANALYSIS_FREEZE_V1.md"
  : IS_SEED1_REPLICATION
    ? "docs/experiments/GLM46V_SEED1_REPLICATION_ANALYSIS_FREEZE_V1.md"
 ...
 */
export const GLM46V_TOKEN_STOP_THRESHOLD = IS_THREE_ARM_EXTENSION || IS_SEED1_REPLICATION ? 4_500_000 : 5_200_000;
/** Informational per-call estimate. Only for planning. */
export const GLM46V_PER_CALL_TOKEN_ESTIMATE = IS_THREE_ARM_EXTENSION || IS_SEED1_REPLICATION ? 1_800 : 5_875;
...
export const GLM46V_TOTAL_TOKEN_QUOTA = IS_THREE_ARM_EXTENSION || IS_SEED1_REPLICATION ? 5_000_000 : 6_000_000;
...
export const GLM46V_TWO_ARM_PLAN_REF = Object.freeze({
  id: IS_THREE_ARM_EXTENSION
    ? "swarmalpha.experiment.v6-fork-glm46v-threearm-seed2-plan-v1"
    : IS_SEED1_REPLICATION
      ? "swarmalpha.experiment.v6-fork-glm46v-twoarm-seed1-plan-v3"
      : "swarmalpha.experiment.v6-fork-glm46v-twoarm-plan-v4",
  version: "1.0.0",
});
...
export const GLM46V_TWO_ARM_MANIFEST_REF = Object.freeze({
  id: IS_THREE_ARM_EXTENSION
    ? "swarmalpha.experiment.v6-fork-glm46v-threearm-seed2-manifest-v1"
    : IS_SEED1_REPLICATION
      ? "swarmalpha.experiment.v6-fork-glm46v-twoarm-seed1-manifest-v3"
      : "swarmalpha.experiment.v6-fork-glm46v-twoarm-manifest-v4",
  version: "1.0.0",
});
export const GLM46V_TWO_ARM_SUMMARY_REF = Object.freeze({
  id: IS_THREE_ARM_EXTENSION
    ? "swarmalpha.experiment.v6-fork-glm46v-threearm-seed2-summary-v1"
    : IS_SEED1_REPLICATION
      ? "swarmalpha.experiment.v6-fork-glm46v-twoarm-seed1-summary-v3"
      : "swarmalpha.experiment.v6-fork-glm46v-twoarm-summary-v4",
  version: "1.0.0",
});
export const GLM46V_TWO_ARM_EXECUTION_REF = Object.freeze({
  id: IS_THREE_ARM_EXTENSION
    ? "swarmalpha.experiment.v6-fork-glm46v-threearm-seed2-execution-v1"
    : IS_SEED1_REPLICATION
      ? "swarmalpha.experiment.v6-fork-glm46v-twoarm-seed1-execution-v3"
      : "swarmalpha.experiment.v6-fork-glm46v-twoarm-execution-v4",
  version: "1.0.0",
});
export const GLM46V_ATTEMPTS_LEDGER_REF = Object.freeze({
  id: IS_THREE_ARM_EXTENSION
    ? "swarmalpha.experiment.v6-fork-glm46v-threearm-attempts-ledger"
    : "swarmalpha.experiment.v6-fork-glm46v-twoarm-attempts-ledger",
  version: "1.0.0",
});
...
// ---------------------------------------------------------------------------
// Plan V4 — the SINGLE call authority (precise per-task agent counts),
// machine-independent (no absolute paths in the hashed body).
// ---------------------------------------------------------------------------

export interface Glm46vTwoArmPlanV4 {
...
export function buildGlm46vTwoArmPlanV4(): Glm46vTwoArmPlanV4 {
  ...
  const body: Omit<Glm46vTwoArmPlanV4, "contentHash"> = {
    ...
    createdAt: IS_THREE_ARM_EXTENSION
      ? "2026-08-21T00:00:00.000Z"
      : IS_SEED1_REPLICATION ? "2026-08-21T00:00:00.000Z" : "2026-08-20T00:00:00.000Z",
  };
  return { ...body, contentHash: hashCanonical(body) };
}

/** Recompute a plan's contentHash from its body (the recorded field excluded). */
export function recomputeGlm46vPlanHash(plan: Glm46vTwoArmPlanV4): string {
 ...
 */
export function freezeGlm46vTwoArmPlanV4(outputDir: string = GLM46V_OUTPUT_DIR): Glm46vTwoArmPlanV4 {
  const plan = buildGlm46vTwoArmPlanV4();
  const target = path.join(outputDir, "plan.json");
  const text = `${JSON.stringify(plan, null, 2)}\n`;
  if (fs.existsSync(target)) {
    const existing = JSON.parse(fs.readFileSync(target, "utf8")) as Glm46vTwoArmPlanV4;
...
export function loadGlm46vTwoArmPlanV4(outputDir: string = GLM46V_OUTPUT_DIR): Glm46vTwoArmPlanV4 {
  const target = path.join(outputDir, "plan.json");
  if (!fs.existsSync(target)) {
    throw new Error("glm46v_plan_not_frozen: run --plan first");
  }
  const plan = JSON.parse(fs.readFileSync(target, "utf8")) as Glm46vTwoArmPlanV4;
...
export interface Glm46vTwoArmExecutionV4 {
...
const SOURCE_BUNDLE_FILES = [
  ...
  ...(IS_SEED1_REPLICATION
    ? ["experiments/campaign/v6/run_v6_fork_glm46v_seed1.ts"]
    : []),
];
...
export function recomputeGlm46vExecutionHash(execution: Glm46vTwoArmExecutionV4): string {
 ...
 */
export function freezeGlm46vTwoArmExecutionV4(
  outputDir: string,
  execution: Glm46vTwoArmExecutionV4,
): Glm46vTwoArmExecutionV4 {
  const target = path.join(outputDir, "execution.json");
  const text = `${JSON.stringify(execution, null, 2)}\n`;
  if (fs.existsSync(target)) {
    const existing = JSON.parse(fs.readFileSync(target, "utf8")) as Glm46vTwoArmExecutionV4;
 ...
 */
export function loadGlm46vTwoArmExecutionV4(
  outputDir: string,
  plan: Glm46vTwoArmPlanV4,
): Glm46vTwoArmExecutionV4 {
  const target = path.join(outputDir, "execution.json");
  ...
  const execution = JSON.parse(fs.readFileSync(target, "utf8")) as Glm46vTwoArmExecutionV4;
...
export interface Glm46vAttemptFinishedV1 {
  ...
  /** Server-returned routing identity, not the client-requested model label. */
  providerModel?: string;
  /** Server-returned request identifier for provider-console reconciliation. */
    ...
    async invoke(request: Readonly<SingleAttemptTextInvokeRequest>, signal: AbortSignal) {
      ...
      appendGlm46vLedgerEvent(input.outputDir, {
        ...
        usage: result.usage?.totalTokens !== undefined
          ...
          : null,
        ...(result.providerMetadata?.model !== undefined
          ? { providerModel: result.providerMetadata.model }
          : {}),
        ...(result.providerMetadata?.requestId !== undefined
          ? { providerRequestId: result.providerMetadata.requestId }
          : {}),
...
export interface Glm46vTwoArmManifestV4 {
...
export function buildGlm46vTwoArmManifestV4(input: {
  plan: Glm46vTwoArmPlanV4;
  execution: Glm46vTwoArmExecutionV4;
  ledgerHash: string;
  taskStatus: Glm46vTaskStatusEntryV1[];
  ...
  files: Glm46vTwoArmManifestFileV1[];
  totalRows: number;
}): Glm46vTwoArmManifestV4 {
  const body: Omit<Glm46vTwoArmManifestV4, "contentHash"> = {
...
export function recomputeGlm46vManifestHash(manifest: Glm46vTwoArmManifestV4): string {
  const { contentHash: _recorded, ...body } = manifest;
  return hashCanonical(body);
}

function writeManifestFile(outputDir: string, manifest: Glm46vTwoArmManifestV4): void {
  const target = path.join(outputDir, "manifest.json");
  const text = `${JSON.stringify(manifest, null, 2)}\n`;
  if (fs.existsSync(target)) {
    const existing = JSON.parse(fs.readFileSync(target, "utf8")) as Glm46vTwoArmManifestV4;
...
export interface Glm46vTwoArmSummaryV4 {
...
export function recomputeGlm46vSummaryHash(summary: Glm46vTwoArmSummaryV4): string {
 ...
 */
export function writeGlm46vSummaryFile(outputDir: string, summary: Glm46vTwoArmSummaryV4): void {
  ...
  const existing = JSON.parse(fs.readFileSync(target, "utf8")) as Glm46vTwoArmSummaryV4;
  if (typeof existing.contentHash === "string" && existing.contentHash === summary.contentHash) return;
  const equivalent = (left: Glm46vTwoArmSummaryV4, right: Glm46vTwoArmSummaryV4): boolean => {
    const strip = (value: Glm46vTwoArmSummaryV4): string => {
...
function buildSummary(input: {
  ...
  startedAt: string;
  completedAt: string;
}): Glm46vTwoArmSummaryV4 {
  const body: Omit<Glm46vTwoArmSummaryV4, "contentHash" | "completedAt"> = {
...
export interface Glm46vTwoArmExecuteResultV4 {
...
export async function executeGlm46vTwoArmGate(input: {
  ...
  plan?: Glm46vTwoArmPlanV4;
}): Promise<Glm46vTwoArmExecuteResultV4> {
  const outputDir = input.outputDir ?? GLM46V_OUTPUT_DIR;
  const plan = input.plan ?? loadGlm46vTwoArmPlanV4(outputDir);
  ...
  let execution: Glm46vTwoArmExecutionV4;
  if (fs.existsSync(executionPath)) {
    execution = loadGlm46vTwoArmExecutionV4(outputDir, plan);
  } else {
    execution = buildGlm46vTwoArmExecutionV4({ plan, repoState, startedAt: clock() });
    freezeGlm46vTwoArmExecutionV4(outputDir, execution);
  }
  ...
  let existingManifest: Glm46vTwoArmManifestV4 | null = null;
  if (fs.existsSync(manifestPath)) {
    existingManifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as Glm46vTwoArmManifestV4;
    ...
    for (const run of runs) {
      ...
      try {
        const rows = await runFork({
          ...
          // V4 scientific completion eligibility: any missing/invalid round-1,
          // round-2, or final report fails the task (no scientific run file).
          completionPolicy: "v4-strict",
        });
        ...
        if (error instanceof V6ProviderInvocationError || error instanceof V6ForkTaskIneligibleError) {
          // Provider failure or scientific-eligibility violation: fail this
          // task with the explicit stage/reason, continue with subsequent tasks.
          const reason = error instanceof V6ForkTaskIneligibleError
            ? `${error.phase}:${error.agentId}:${error.code}`
            : error.code;
          appendGlm46vLedgerEvent(outputDir, {
            type: "task_failed", runId: run.runId, taskId: run.taskId, seed: run.seed,
            reason, ts: clock(),
          });
          console.warn(`[fork] task failed ${run.runId}: ${reason} — continuing with subsequent tasks`);
          continue;
        }
        throw error;
      }
  ...
  if (!halted) {
    ...
    if (!existingManifest) {
      ...
      const manifest = buildGlm46vTwoArmManifestV4({
        plan,
        execution,
        ledgerHash,
...
export function verifyGlm46vTwoArmOutput(outputDir: string = GLM46V_OUTPUT_DIR): { ok: boolean; detail: string } {
  ...
  let plan: Glm46vTwoArmPlanV4 | null = null;
  if (!fs.existsSync(planPath)) {
    problems.push("plan.json missing");
  } else {
    plan = JSON.parse(fs.readFileSync(planPath, "utf8")) as Glm46vTwoArmPlanV4;
  ...
  // ---- execution identity ----
  const executionPath = path.join(outputDir, "execution.json");
  let execution: Glm46vTwoArmExecutionV4 | null = null;
  if (!fs.existsSync(executionPath)) {
    problems.push("execution.json missing");
  } else {
    execution = JSON.parse(fs.readFileSync(executionPath, "utf8")) as Glm46vTwoArmExecutionV4;
    if (recomputeGlm46vExecutionHash(execution) !== execution.contentHash) {
  ...
  if (!fs.existsSync(manifestPath)) {
    ...
    const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as Glm46vTwoArmManifestV4;
    ...
    if (!fs.existsSync(summaryPath)) {
      ...
      const summary = JSON.parse(fs.readFileSync(summaryPath, "utf8")) as Glm46vTwoArmSummaryV4;
...
function runPlan(): number {
  const plan = freezeGlm46vTwoArmPlanV4();
  console.log(`GLM-4.6V ${IS_THREE_ARM_EXTENSION ? "three-arm extension" : "two-arm cross-model gate"} plan (frozen)`);
  ...
  console.log(`  arms:                     ${plan.arms.join(" + ")}`);
...
export async function runGlm46vTwoArmCli(): Promise<number> {
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  runGlm46vTwoArmCli().then(code => { process.exitCode = code; }).catch(error => {
    console.error(error instanceof Error ? error.message : String(error));
    process.exitCode = 4;
  });
}
